...
# Validar parâmetros
if ($JdbcBatchSize -le 0 -or $JdbcBatchIntervalMs -le 0) {
  throw "JdbcBatchSize e JdbcBatchIntervalMs devem ser positivos"
}

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$runDir = Join-Path $OutputRoot "$timestamp-batch$JdbcBatchSize-interval$JdbcBatchIntervalMs"
...
$metadata = [ordered]@{
  experiment_type = "jdbc_variation"
  jdbc_batch_size = $JdbcBatchSize
  jdbc_batch_interval_ms = $JdbcBatchIntervalMs
...
# Cada execução precisa de Kafka e TimescaleDB vazios para evitar reprocessamento
# e agregação de eventos pertencentes a configurações anteriores.
...
Write-Host "Iniciando Docker Compose com JDBC_BATCH_SIZE=$JdbcBatchSize, JDBC_BATCH_INTERVAL_MS=$JdbcBatchIntervalMs"
...
$env:JDBC_BATCH_SIZE = [string]$JdbcBatchSize
$env:JDBC_BATCH_INTERVAL_MS = [string]$JdbcBatchIntervalMs
...
try {
  ...
  Write-Host "Parando coleta de métricas..."
...
# Coletar resultados de queries
Write-Host "Coletando métricas de TimescaleDB..."

# Query principal: latência por bucket (10 segundos)
...
# Captura de estado final do Docker
...
# Atualizar metadados com conclusão
...
Write-Host "Experimento JDBC variation completo. Resultados salvos em: $runDir"
Write-Host ""
Write-Host "Arquivos gerados:"
Write-Host "  - $latencyBucketsCsv (latência por bucket de 10s)"
Write-Host "  - $latencySummaryCsv (latência agregada)"
Write-Host "  - $throughputCsv (throughput efetivo)"
Write-Host "  - $statsSamples (docker stats a cada ${StatsIntervalSeconds}s)"
Write-Host "  - $flinkMetricsPath (métricas Flink)"
